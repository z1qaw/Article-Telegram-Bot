...
class ArabianBusinessParser:
    ...
    def get_latest_by_tag(self, tag):
        ...
        article_block = soup.find('div', {'class': re.compile('custom-trending-tag')})
        article_list = article_block.find_all('div', {'class': re.compile('trd-tags')})

        items = {'tag': tag,
                 'article_list': []}

        for item in article_list:
            item_link = item.find('h3', {'class': re.compile('main-lead-text')}).find('a').get('href')
            if item_link:
    ...
    def get_latest(self):
        print('ArabianBusinessParser: Get new articles list...')
    ...
    def get_article(self, uri):
        print('ArabianBusinessParser: Get article: ' + uri)
        ...
        try:
            article_title = soup.find('h1', {'class': re.compile('news-title|main-bg-title')}).text
        except:
            article_title = None
        try:
            text_blocks = soup.find('div', {'class': 'news-data'}).find_all('p')
        except:
            text_blocks = []
        ...
        try:
            article_pub_date = soup.find('div', {'class': re.compile('date-published')}).find('span', {'class': re.compile('d-t')}).text
        except:
        ...
        article_images = []
        article_images_blocks = soup.find_all('img', {'class': re.compile('media-element')})
